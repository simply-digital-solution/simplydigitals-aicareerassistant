...
import type { ResearchOutput, JobOpportunity, ProfileData, StoredJobsResponse, StoredJob } from '../api/client'
...
function LatestJobs({ feedbackMap, onFeedback }: {
  ...
  // Collect unique roles from all pages for the filter dropdown
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      await qc.invalidateQueries({ queryKey: ['stored-jobs'] })
      await qc.invalidateQueries({ queryKey: ['stored-jobs-all-roles'] })
    } finally {
      setRefreshing(false)
    }
  }
...
export default function ResearchPanel() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      {/* ── Latest Jobs (automated) ── */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <LatestJobs feedbackMap={feedbackMap} onFeedback={handleFeedback} />
      </div>

      {/* ── Divider ── */}
      <div className="flex items-center gap-3">
        <div className="flex-1 border-t border-gray-200" />
        <span className="text-xs text-gray-400 font-medium uppercase tracking-wide">Manual Search</span>
        <div className="flex-1 border-t border-gray-200" />
      </div>
